...
export class HomePage implements OnInit {
  ...
  inicialization() {
    if (this.moviedbService.empty()) {
      ...
      for (let index = 0; index < this.moviesinit.length; index++) {
        console.log("REllenenando")
        this.moviedbService.setItem(this.moviesinit[index].id, this.moviesinit[index]);
      }

      //NO ME FUNCIONA EN FOREACH
      /*
